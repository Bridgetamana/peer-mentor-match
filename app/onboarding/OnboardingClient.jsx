...
export default function OnboardingClient({ userName }) {
  ...
  const handleSubmit = async (payload) => {
    ...
    try {
      ...
      window.location.href = profile?.role === 'tutor' ? '/dashboard/tutor' : '/dashboard/learner';
    } catch (e) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-2xl mx-auto px-6 py-12">
        ...
        {selectedRole === 'learner' && (
          <LearnerForm onBack={handleBack} onSubmit={handleSubmit} submitting={submitting} />
        )}

        {selectedRole === 'tutor' && (
          <TutorForm onBack={handleBack} onSubmit={handleSubmit} submitting={submitting} />
        )}
      </main>
    </div>
  );
